...
import { updateCreditNote } from "../../../../Redux/creditNote/creditNoteSlice";
import { toast, ToastContainer } from "react-toastify";
import { useNavigate } from "react-router-dom";

const LinkInvoiceDetails = ({ isFutureCredit, backToList, creditNoteData }) => {
  ...
  const handleCreditNoteComplete = () => {
    ...
    dispatch(
      updateCreditNote({
        creditNoteId: creditNoteData?.creditNoteId,
        payload: {
          status: "Closed",
        },
      })
  ...
  const navigateToInvoice = () => {
    navigate('/admin/sales-invoice', {state: {creditNoteData}});
  }

  return (
    <div>
      {openCreateInvoice ? (
        <SalesReInvoice backToList={backToList} />
      ) : (
        <div className="purchase-list">
          <h2>{creditNoteData?.creditNoteId}</h2>
